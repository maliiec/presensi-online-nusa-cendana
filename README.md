[aplikasi_presensi_online (1).html](https://github.com/user-attachments/files/32574116/aplikasi_presensi_online.1.html)
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Sistem Presensi Online Karyawan</title>
    <!-- Tailwind CSS CDN -->
...
<body class="bg-slate-100 text-slate-800 min-h-screen flex flex-col antialiased">
    ...
    <header class="bg-indigo-700 text-white shadow-lg sticky top-0 z-30">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex justify-between items-center">
            <div class="flex items-center space-x-3">
                <div class="p-2.5 bg-indigo-600 rounded-xl text-xl shadow-inner border border-indigo-500/50">
                    <i class="fa-solid fa-clipboard-user"></i>
                </div>
                <div>
                    <h1 class="text-lg sm:text-xl font-black tracking-tight">PresensiKu</h1>
                    <p class="text-xs text-indigo-200" id="header-subtitle">Portal Kehadiran Karyawan</p>
                </div>
            </div>
            
            <div class="flex items-center gap-3">
                <!-- Clock Display -->
                <div class="hidden md:flex flex-col text-right text-xs text-indigo-100 pr-2 border-r border-indigo-500/40">
                    <span id="live-time" class="font-bold text-sm text-white">00:00:00</span>
                    <span id="live-date" class="text-[11px] text-indigo-200">24 Sep 2026</span>
                </div>

                <!-- Admin Mode Toggle Button -->
                <button id="btn-mode-toggle" onclick="toggleAdminMode()" class="flex items-center gap-2 bg-indigo-800 hover:bg-indigo-900 text-white px-3.5 py-2 rounded-xl text-xs font-bold transition border border-indigo-500/40 shadow-sm active:scale-95">
                    <i class="fa-solid fa-user-gear" id="icon-mode"></i>
    ...
    <main class="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div id="view-karyawan" class="max-w-xl mx-auto space-y-5">
            ...
            <!-- Information Banner -->
            <div class="bg-gradient-to-r from-indigo-600 to-indigo-800 text-white p-5 rounded-2xl shadow-md flex items-center justify-between">
                <div>
                    <h2 class="text-base font-bold">Form Kehadiran Harian</h2>
                    <p class="text-xs text-indigo-200 mt-1">Isi data presensi Anda dengan benar hari ini.</p>
                </div>
            ...
            <div class="bg-white p-6 sm:p-7 rounded-2xl shadow-sm border border-slate-200">
                <form id="attendance-form" onsubmit="submitAttendance(event)" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3" id="time-inputs-container">
                        <div>
                            <div class="flex justify-between items-center mb-1">
                                <label for="jam-masuk" class="text-[11px] font-bold text-slate-700 uppercase tracking-wider">Jam Masuk</label>
                                <button type="button" onclick="setExactTime('jam-masuk')" class="text-[10px] text-indigo-600 font-bold hover:underline">Jam Sekarang</button>
                            </div>
                            <input type="time" id="jam-masuk" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition">
                        </div>
                        <div>
                            <div class="flex justify-between items-center mb-1">
                                <label for="jam-keluar" class="text-[11px] font-bold text-slate-700 uppercase tracking-wider">Jam Keluar</label>
                                <button type="button" onclick="setExactTime('jam-keluar')" class="text-[10px] text-indigo-600 font-bold hover:underline">Jam Sekarang</button>
                            </div>
                            <input type="time" id="jam-keluar" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition">
                        </div>
                    </div>
                    ...
                    <!-- Submit Button -->
                    <div class="pt-2">
                        <button type="submit" id="btn-submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3.5 px-4 rounded-xl font-bold text-sm transition shadow-lg hover:shadow-indigo-200 flex justify-center items-center gap-2 active:scale-[0.99]">
        ...
        <div id="view-admin" class="hidden space-y-6">
            ...
            <div class="bg-slate-900 text-white rounded-2xl p-4 sm:p-5 shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <div class="flex items-center gap-2">
                        <span class="px-2.5 py-0.5 bg-indigo-600 text-white rounded-md text-[10px] font-bold uppercase tracking-wider">Mode Pengawasan</span>
                        <h2 class="text-base font-bold">Dashboard Pemantauan Absensi Karyawan</h2>
                    </div>
                    <p class="text-xs text-slate-300 mt-1">Pantau seluruh data kehadiran karyawan, filter laporan, dan kelola integrasi Google Sheet.</p>
                </div>
    ...
    <!-- MODAL 3: Integrasi Google Sheet -->
        ...
        <div class="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div class="flex justify-between items-center border-b pb-3 mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-lg font-bold text-slate-800">Pengaturan Google Sheet Pusat</h3>
                </div>
            ...
            <div class="space-y-4 text-xs">
                <p class="text-slate-600 leading-relaxed">
                    Setiap kali karyawan mengisi form, data presensi akan **otomatis dikirimkan langsung ke 1 Google Sheet pusat Anda**.
                </p>

                <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                    <h4 class="font-bold text-slate-800 text-sm">Cara Menghubungkan Google Sheet:</h4>
                    <ol class="list-decimal pl-4 space-y-2 text-slate-700">
                        <li>Buka Google Sheet baru di <a href="https://sheets.new" target="_blank" class="text-indigo-600 underline font-bold">sheets.new</a></li>
                        <li>Buat judul kolom di baris 1: <br><span class="bg-slate-200 px-1.5 py-0.5 rounded font-mono text-[11px] text-slate-800">ID | Tanggal | Nama | Jabatan | Departemen | Jam Masuk | Jam Keluar | Status | Catatan</span></li>
                        <li>Klik menu **Extensions (Ekstensi)** &gt; **Apps Script**.</li>
                        <li>Hapus semua kode bawaan, lalu **Copy & Paste Skrip** di bawah ini:</li>
                    </ol>

                    <div class="relative">
                        <textarea id="apps-script-code" readonly class="w-full h-36 bg-slate-900 text-emerald-400 font-mono text-[11px] p-3 rounded-xl outline-none resize-none">
function doPost(e) {
  try {
    ...
    return ContentService.createTextOutput(JSON.stringify({"result": "error", "error": err.toString()}))
      .setMimeType(ContentService.MimeType.JSON);
                        ...
                        <button onclick="copyAppsScript()" class="absolute top-2 right-2 bg-indigo-600 hover:bg-indigo-700 text-white px-2.5 py-1 rounded-lg text-[10px] font-bold transition">
                            <i class="fa-regular fa-copy mr-1"></i>Salin Kode
                        </button>
                    </div>

                    <ol start="5" class="list-decimal pl-4 space-y-1.5 text-slate-700">
                        <li>Klik **Deploy (Terapkan)** &gt; **New deployment (Penerapan Baru)**.</li>
                        <li>Pilih Jenis: **Web App (Aplikasi Web)**.</li>
                        <li>Setel **Who has access** menjadi: **Anyone (Siapa Saja)**. Lalu klik Deploy.</li>
                        <li>Salin **Web App URL** dan tempelkan pada kolom di bawah.</li>
                    </ol>
                </div>

                <div>
                    <label for="webhook-url" class="block font-bold text-slate-800 mb-1">Google Apps Script Webhook URL:</label>
                    <input type="url" id="webhook-url" placeholder="https://script.google.com/macros/s/AKfycbx.../exec" class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl text-xs outline-none focus:ring-2 focus:ring-indigo-500">
                </div>

                <div id="connection-status" class="hidden p-2.5 rounded-xl text-xs"></div>

                <div class="flex justify-end gap-2 pt-2 border-t">
                    <button type="button" onclick="testConnection()" class="px-4 py-2 border border-slate-300 text-slate-700 rounded-xl hover:bg-slate-100 font-semibold">
                        Uji Sinyal
                    </button>
                    <button type="button" onclick="saveWebhookUrl()" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow-md">
                        Simpan Webhook URL
                    </button>
                </div>
            </div>
    ...
    <!-- Notification Toast -->
    <div id="toast" class="fixed bottom-5 right-5 z-50 transform translate-y-20 opacity-0 transition-all duration-300 pointer-events-none flex items-center gap-2.5 bg-slate-900 text-white px-4 py-3 rounded-xl shadow-xl text-xs">
        <i id="toast-icon" class="fa-solid fa-circle-check text-emerald-400 text-base"></i>
        <span id="toast-msg">Notifikasi Toast</span>
    </div>
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('jam-masuk').value = "08:00";
            document.getElementById('jam-keluar').value = "17:00";
        ...
        function updateViewMode() {
            ...
            if (isAdminLoggedIn) {
                ...
                btnModeToggle.className = "flex items-center gap-2 bg-indigo-800 hover:bg-indigo-900 text-white px-3.5 py-2 rounded-xl text-xs font-bold transition border border-indigo-500/40 shadow-sm";
                iconMode.className = "fa-solid fa-user-gear";
                labelMode.textContent = "Akses Admin";
                headerSubtitle.textContent = "Portal Kehadiran Karyawan";
            }
        }
